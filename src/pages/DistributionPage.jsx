...
function DistributionPage() {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await API.markDistributionAsPendingDelete(id, reason);
    ...
    return (
        <div className="flex flex-col items-center justify-center">
            ...
            <div className="bg-white/90 backdrop-blur-xl shadow-2xl rounded-3xl p-6 sm:p-8 max-w-4xl w-full">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-300">
                            <thead className="bg-gray-200/70">
                                <tr>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase">School</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase">Amount (RWF)</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {distributions.map(dist => (
                                    <tr key={dist.id} className={dist.delete_status === "pending" ? "bg-yellow-100" : ""}>
                                        <td className="px-6 py-4 text-sm font-medium text-gray-800">{dist.school_name}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{parseFloat(dist.amount).toFixed(2)}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{new Date(dist.distributed_on).toLocaleString()}</td>
